...
function connectToLive(username, reconnectCount = 0) {
    ...
    // Comentarios de chat
    let chatDebugLogged = false;
    conn.on('chat', (data) => {
        const { nickname, uniqueId } = extractUser(data);
        const comentario = data.comment ?? data.content ?? data.text ?? '';
        if (!chatDebugLogged) {
            log.info(`[debug chat event keys] ${Object.keys(data).join(', ')}`);
            chatDebugLogged = true;
        }
        if (!comentario) {
            log.warn(`[${username}] Comentario vacio recibido de ${uniqueId || nickname}, se ignora`);
            return;
        }
        enqueue(username, uniqueId || nickname, comentario);
    });
